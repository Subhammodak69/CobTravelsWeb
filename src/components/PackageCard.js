...
export default function PackageCard({ pack, index }) {
  ...
  return (
    <article className={`group animate-fade-up cursor-pointer overflow-hidden rounded-[1.75rem] border border-white/70 bg-white shadow-xl shadow-slate-950/5 transition duration-500 hover:-translate-y-2 hover:shadow-glow motion-reduce:animate-none ${index % 3 === 1 ? "[animation-delay:120ms]" : index % 3 === 2 ? "[animation-delay:240ms]" : ""}`} onClick={() => selectPackage(pack.id)} onMouseEnter={playVideo} onMouseLeave={stopVideo}>
      ...
      <div className="p-5 sm:p-6">
        ...
        <h3 className="mb-6 font-display text-2xl font-semibold leading-tight tracking-tight text-slate-950 sm:text-3xl">{pack.title}</h3>
        <div className="flex items-center justify-between gap-4 text-xs font-semibold text-slate-500"><span>{pack.destination}</span><span>{pack.type}</span></div>
      </div>
    </article>
  );
}
